...
class PlaneScene(LinearTransformationScene):
    ...
    def escalando(self):
        ...
        label = vector.coordinate_label(color = RED_A)
        ...
        name = always_redraw(lambda: MathTex(r"n \cdot \vec{v}", color = RED_A).shift((vector.get_end() - vector.get_start()) / 2).shift(UP * 0.5))

        self.play(Create(vector), Create(punto), Create(name), Create(punto))
        
        #Cambiando los valores de n
        self.play(n.animate.set_value(1), run_time = 3)
        self.wait(1)  
        ...
        self.play(FadeOut(vector), FadeOut(punto), FadeOut(name))
